...
\author{John D. Storey\thanks{\url{http://genomine.org/contact.html}}, Jeffrey T. Leek, Andrew J. Bass}
...
{\tt edge} was first released in 2005 and described in the publication \cite{leek2005}.  It was an independently released R package by the John Storey Lab, which included multi-threading and a graphical user interface.  However, the current version is now available through Bioconductor as a standard R package.
...
edge_obj <- edgeModel(data = kidexpr, cov = cov, nullMod = null_model, altMod = full_model)
@
...
edge_obj <- edgeModel(data = gibson$gibexpr, cov = cov, altMod = null_model, nullMod = null_model)
@
...
edge_obj <- edgeModel(data = kidexpr, cov = cov, altMod = null_model, nullMod = null_model)
@
...
edge_obj <- edgeModel(data = endotoxin$endoexpr, cov = cov, altMod = null_model, nullMod = null_model)
@
...
null_model <- ~sex
full_model <- ~sex + ns(age, df=4)
edge_obj <- edgeModel(data = kidexpr, cov = cov, altMod = full_model, nullMod = null_model)
@
