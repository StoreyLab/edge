...
{\bfseries Step 2:} Use the function {\tt edgeModel} to create an {\tt edgeSet} object:
<<edgeModel_endo, dependson="crtx_endo">>=
edgeObj <- edgeModel(data=expr, ind=ind, tme=time, grp=class, sampling="timecourse", basis.type="ncs", basis.df=4)
@
...
library(ggplot2)
class <- as.factor(cls) 
qplot(tme, fitVals[1,], color=class, ylab="expression", xlab="time")
@
...
edgeObj <- edgeModel(data=expr, adj.var=model.matrix(~batch + gender), grp=location, sampling="static")
@
