...
<<make_eSet, echo=TRUE>>=
edgeObj <- edgeModel(data=kidexpr, adj.var=sex, tme=age, sampling="timecourse", basis.type="ncs", basis.df=4)
@
...
edgeObj <- edgeModel(data=gibson$gibexpr, adj.var=NULL, grp=gibson$location, sampling="static")
@
...
\item {\tt bio.var} Biological variables
...
nullModel(edgeObj)
@
The {\tt adj.var} corresponds to the adjustment variables, the {\tt grp} corresponds to the various location groups in the experiment. The key slot in {\tt edgeObj} is the {\tt qvalueObj} which should be the only empty slot. The other slots are directly related to the input data and hypothesis models. See the section~\ref{sec:edgeSet} for more details on the {\tt edgeSet} object.
...
qplot(location, fitVals[1,], geom="boxplot", colour=gender, xlab= "location", ylab="expression")
@
