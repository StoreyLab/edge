...
If the alternative and null models are known to the user then {\tt build\_models} can be used to make an {\tt deSet} object:
...
Alternatively, if the user is familiar with their alternative and null models in the study then {\tt build\_models} can be used to input the models directly:
...
The variable {\tt edge\_obj} is an {\tt deSet} object that stores all the relevant experimental data. The {\tt deSet} object is discussed further in the next section. 
...
The {\tt fit\_models} function is an implementation of least squares using the alternative and null models:
...
nullModel(de_obj)
@
See \cite{storey:2005} for how the models in the {\tt endotoxin} experiment are formed. Alternatively, if the user is familiar with their alternative and null models in the study then {\tt build\_models} can be used to input the models directly:
...
See {\tt ?deFit} for more details on accessing the slots in an {\tt deFit} object. The fitted values of the second gene is shown in Figure~\ref{fig:eplotFit}. The null model fit is the average expression. It appears that the alternative model fits a pattern that might be observed in the raw data. Next, we have to test whether the observed differences between the model fits are significant.
...
To find the surrogate variables and add them to the experimental models in {\tt edge\_obj}, use the function {\tt apply\_sva}:
<<sva>>=
...
After {\tt odp} or {\tt lrt} is used, the user may wish to change some parameters used when calculating the q-values. This can be done by using the {\tt apply\_qvalue} function. Lets review the analysis process for the {\tt kidney} dataset in~\ref{sec:kidney}: create the alternative and null models and then run {\tt odp} or {\tt lrt} to get significance results. Applying these steps in the {\tt kidney} dataset:
