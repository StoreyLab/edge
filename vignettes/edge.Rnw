...
A methodology for analyzing time course microarray data is introduced and applied to two studies on human. 
...
To import the data use the {\tt data} function:
<<import_data_gib>>=
data(gibson)
names(gibson)
@
There are a few variables in the data set: {\tt batch}, {\tt gibexpr}, {\tt gender}, and {\tt location}. The three covariates of interest are {\tt gender}, {\tt batch} and {\tt location}. The {\tt location} variable contains information on where individuals are sampled: ``VILLAGE'', ``DESERT'' or ``AGADIR''. The {\tt gender} variable specifies whether the individuals is a male or female and there are two different {\tt batches} in the study. The {\tt gibexpr} variable contains the expression matrix of the experiment.
...
qplot(location, gibson$gibexpr[1,], geom="boxplot", colour=gender:batch, xlab= "location", ylab="expression")
@

It would be careless to conclude that individuals from ``VILLAGE'' are less expressed than other locations. Instead, the data needs to be adjusted with the covariates in the study.

\subsection{Creating the alternative and null models}
In the {\tt gibson} study, we are interested in finding differentially expressed genes within each location. There are two ways to input the experimental models in {\tt edge}: {\tt edgeModel} and {\tt edgeStudy}. {\tt edgeModel} should be used by users unfamiliar with formulating the alternative and null models but are familiar with the covariates in the study: 
...
The {\tt adj.var} is for the adjustment variables, {\tt grp} is the variable containing the group assignments for each individual in the study and {\tt sampling} describes the type of experiment. Since {\tt gibson} is a static study, the {\tt sampling} argument will be ``static''. The {\tt grp} variable will be the location variable in the study and the adjustment variables are gender and batch.
...
The {\tt cov} argument is a data frame of all the relevant covariates, {\tt altMod} and {\tt nullMod} are the alternative and null models of the experiment. The null model contains the gender and batch covariables and the alternative model includes the location variable. Notice that the models must be formatted as a formula and contain the same variable names as in the {\tt cov} data frame. The variable {\tt edgeObj} is an {\tt edgeSet} object that stores all the relevant experimental data. It is discussed further in the next section. 

\subsection{The {\tt edgeSet} object}
Once either {\tt edgeModel} or {\tt edgeStudy} is used, an {\tt edgeSet} object is created. To view some of the slots contained in the object:
<<edgeObjSlots_gib>>=
slotNames(edgeObj)
@
...
As an example of how to access the slots of {\tt edgeObj} lets say we are interested in viewing the alternative and null models. The models can be accessed by:
<<gibson_acc_fm, eval=TRUE, dependson="edgeModel_gib">>=
fullModel(edgeObj)
nullModel(edgeObj)
@
Now that the models have been verified, we can extract them in matrix form for additional analysis:
...
@
Interpreting the models for hypothesis testing is very intuitive: Does the alternative model better fit the data when compared to the null model? 
...
\label{sec:gibson}
The {\tt kidney} dataset is an independent time course study. The goal of the experiment is to identify genes that show ``within-class temporal differential expression'', i.e., genes that show statistically significant changes in expression over time. The study measured gene expression in the kidney from 72 human subjects ranging in age from 27 to 92 years. There is only one array per sample and the subjects age and tissue type are recorded. 
...
There are two covariates in the data set: {\tt sex} and {\tt age}. The {\tt sex} variable is whether the subject was male, ``m'', or female, ``f'', and the {\tt age} variable is the age of the patients. 
