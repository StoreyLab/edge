...
edgeObj <- edgeModel(data=gibson$gibexpr, adj.var=data.frame(gibson$gender, gibson$batch), grp=gibson$location, sampling="static")
@
...
{\bfseries Step 2:} Use the function {\tt edgeModel} to create an {\tt edgeSet} object:
<<edgeModel_endo>>=
edgeObj <- edgeModel(data=endotoxin$endoexpr, ind=endotoxin$ind, tme=endotoxin$time, grp=data.frame(endotoxin$class), sampling="timecourse", basis.type="ncs", basis.df=4)
@
